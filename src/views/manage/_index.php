...
<script type="text/javascript">
        ...
        table = $('#itemtable').DataTable( {
            ...
            drawCallback: function ( settings ) {
                ...
                api.column(colGroup, {page:'current'} ).data().each( function ( group, i ) {
                    if ( last !== group ) {
                        $(rows).eq( i ).before(
                            '<tr class="group"><td colspan="4"><img src="<?= Yii::getAlias('@web')?>/images/datatable/'+group+'.png" class="datatableicon">&nbsp;'+group+'</td></tr>'
                        );
            ...
            columnDefs: [
                ...
                { targets: colStatus, visible: false },
                {
                    ...
                    render: function ( data, type, row ) {
                        if(data) {
                            return '<?=Html::img('@web/images/datatable/danger.png', [
                                'class' => 'datatableicon',
                                'alt'   => 'warning',
                                'title'   => 'warning',
                            ]) ?>';
    ...
    $(document).ready(function () {
        ...
        foreach ($statusmap as $status=>$style)
        {
            echo "$('#cb_filter_".$status."').prop(\"checked\", settingsLoadBool('filter.".$status."', true));\n";
        }
        ?>
    ...
    function sql2file() {
        ...
        $.ajax({
            url: '<?php echo Yii::$app->getUrlManager()->createUrl('db/sql2file'); ?>',
            data: formData,
            processData: false,
            contentType: false,
            type: 'POST',
    ...
    function file2sql() {
        ...
        $.ajax({
            url: '<?php echo Yii::$app->getUrlManager()->createUrl('db/file2sql'); ?>',
            data: formData,
            processData: false,
            contentType: false,
            type: 'POST',
    ...
    function autogen() {
        $.ajax({
            url: '<?php echo Yii::$app->getUrlManager()->createUrl('db/autogen'); ?>',
            processData: false,
            contentType: false,
            type: 'POST',
            success: function () {
    ...
    function updateItemTable() {
        ...
        foreach ($statusmap as $status=>$style)
        {
            echo "settingsSave('filter.".$status."', $('#cb_filter_".$status."').prop(\"checked\"));\n";
        }
        ?>
